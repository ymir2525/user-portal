...
// FIX: Import the helper function from PrescriptionForm
import PrescriptionForm, { formatPrescribedItemsAsText } from "../../components/PrescriptionForm"; 
...
      // ✅ keep the same keys BHW uses
      contact_number: data.patients?.contact_number ?? "",
      contact_person_number: data.patients?.contact_person ?? "",   // 09xxxxxxxxx
...
// --- Realtime: keep chart in sync if BHW/Admin/Nurse edits the patient row ---
useEffect(() => {
...
// (optional) Keep in sync if the same record is updated (e.g., updated_at bump)
useEffect(() => {
...
  // You need to call the original onSavePdf logic which is embedded in the component calling this.
  // Since this is a DoctorQueueChart component, we need to manually replicate the document saving logic here.
  try {
    const form = payload; // payload is the form data
